# GhostSpectator
A plugin for the SCP-SL game, that allows player to become a Ghost: Tutorial, that is undetectable to alive players and has no influence on a course of the game. Depending on the config, Ghosts can perform various activities, such as teleport to alive players, practice target shooting or challenge another Ghost to a duel.

## Features
- Ghosts can pass through most of the doors.
- Ghosts can teleport to a random alive player by dropping a ghost item, which type is declared in the config. If the type is set to None, then the player won't receive a ghost item. There is an option to exclude certain roles that Ghosts won't be able to teleport to.
...
- Ghosts are 10x smaller than normal players.
...
### ghostspectator
Parent command. Type an empty command for more information regarding subcommands. Subcommands:
...
### duel
Parent command. Type an empty command for more information regarding subcommands. Subcommands:
- accept - Accept a duel offer from a player. Provide a player nickname, whole or part of it, otherwise the first offer will be accepted. The case is ignored.
- cancel - Cancel your duel request or current duel.
- list (duel) - Print a list of all players who challenged you to a duel.
- player - Challenge another Ghost to a duel by typing their nickname, whole or part of it. The case is ignored. 
- reject - Reject a duel offer from player(s). Provide a player nickname, whole or part of it, otherwise all offers will be rejected. The case is ignored.
...
## Permissions
- gs.duel - allows the player to use *duel* and *accept* commands
- gs.firearm - allows the player to use *givefirearm* command
- gs.item - allows the player to drop and throw items (expect ghost item) and use Particle Disruptor
- gs.list - allows the player to use *list* (RA) command
- gs.listen.dead - allows the player to listen Spectators (via command)
- gs.autolisten.dead - allows the above automatically, when spawned as Ghost
- gs.listen.ghost - allows the player to lsiten to other Ghosts via RoundSummary chat, if they are not within Proximity chat or further than configurable distance (via command)
- gs.autolisten.ghost - allows the above automatically, when spawned as Ghost
- gs.listen.scp - allows the player to listen to SCPs (via command)
- gs.autolisten.scp - allows the above automatically, when spawned as Ghost
- gs.noclip - allows the player to have noclip permitted
- gs.spawn.other - allows the player to use *spawn* and *despawn* commands
- gs.spawn.self - allows the player to use *ghostme* command
- gs.target - allows the player to use *createtarget* command
- gs.warhead - allows the player to remain Ghost and use *ghostme* and *spawn* commands after warhead detonation
